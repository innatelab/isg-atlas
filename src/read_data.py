...
def read_flrsc_data(flrsc_path,id2virus):
    # flrsc_txt_files=os.listdir(data_path+'flrsc/')
    ...
    for flrsc_txt_file in flrsc_txt_files:
        ...
        # if mock_plate_id is not np.nan:
        #     mock_df=read_incucyte_txt_file(data_path+'mock-fi/'+mock_plate_id+'.txt')
        #     mock_df.columns=pd.MultiIndex.from_product([['mock'],[virus], [set_], [bio_rep], [mock_plate_id],mock_df.columns],names=['reading','virus','set','bio_rep','mock_plate_id','knockout'])
        #     dfs.append(mock_df)
        # else:
        #     print(virus,set_,bio_rep,flrsc_txt_file)
    # df=pd.concat(dfs, axis=1).sort_index(axis=1,ascending=True)
    # return df
    return pd.concat(dfs, axis=1).sort_index(axis=1,ascending=True).sort_index()
